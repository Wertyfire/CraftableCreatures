...
public class CCGuiHandler implements IGuiHandler {
    ...
    public Object getGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
        if (ID == mod_CraftableCreatures.guiSoulExtractor)
            return new GuiSoulExtractor(player.inventory, (TileEntitySoulExtractor) world.getBlockTileEntity(x, y, z));
        else if (ID == mod_CraftableCreatures.guiCombiner)
            return new GuiCombiner(player.inventory, (TileEntityCombiner) world.getBlockTileEntity(x, y, z));
        else if (ID == mod_CraftableCreatures.guiGuideBook)
