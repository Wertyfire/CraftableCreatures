...
public class mod_CraftableCreatures extends NetworkMod {
    ...
    private static final String enUSTranslations = "/craftablecreatures/lang/en_US.lang";
    private static final String ruRUTranslations = "/craftablecreatures/lang/ru_RU.lang";
    ...
    public boolean onTickInGame(float time, Minecraft minecraftInstance) {
        ...
        EntityPlayer player = minecraftInstance.thePlayer;

        if (player != null) {
            if (checkForUpdates && !player.worldObj.isRemote && !updateNotified) {
                ...
                if (CCVersionChecker.getStatus() == CCVersionChecker.UpdateResult.OUTDATED) {
                    ...
                    System.out.println(StringTranslate.getInstance().translateKey("craftableCreatures.chat.outdated"));
                    player.addChatMessage(StringTranslate.getInstance().translateKey("craftableCreatures.chat.getUpdate") + " " + homepage);
                    ChatUtils.sendUpdateChangelogMessage(player, CCVersionChecker.getChangelog(), StringTranslate.getInstance().getCurrentLanguage());
                    updateNotified = true;
                }
    ...
    private static Properties loadLangProperties(String path) {
        ...
        try {
            ...
            err("Error loading translations from '" + path + "': " + e.getMessage());
        }

        return properties;
    }

    private static void addTranslations(Properties translations, String lang) {
        for (Object key_ : translations.keySet()) {
            String key = (String) key_;
            ModLoader.addLocalization(key, lang, translations.getProperty(key));
        }
    }

    private static void loadBlocks() {
        bluestoneBlock = createBlock(new BlockDefault(bluestoneBlockID, 0, Material.iron, 3f, 5f, Block.soundMetalFootstep)
                .setBlockName("bluestoneBlock"), "pickaxe", 2);
        bluestoneOre = createBlock(new BlockOre(bluestoneOreID, 1, 3f, 5f)
    ...
    private static void loadItems() {
        ...
        ocelotTail = new ItemDefault(ocelotTailID).setIconCoord(13, 1).setItemName("ocelotTail");
        ...
        for (int i = 1; i < 22; i++) ItemSoulElement.soulTextures[i] = 4 + i;
    }
