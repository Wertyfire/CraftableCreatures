...
public class GuiScreenGuideBook extends GuiScreen {
    ...
    private void drawPageText(int mouseX, int mouseY) {
        ...
        if (currentPage == 2) {
            ...
            drawEmptyString();
            drawSplitString(f("craftableCreatures.guide.page4.mainText.1", new Object[]{EnumSoulElement.values().length, EnumSoulElement.values().length - 1}));
            ...
            drawItemStack(s(CCItems.blueprint), lineXPos + xSize - 18 - 32, lineYPos - fontHeight);
            ...
            drawItemStack(s(CCItems.spawn_egg_blueprint), lineXPos + xSize - 18 - 32, lineYPos - fontHeight * 2);
            ...
            drawCraftingRecipe(new ItemStack[]{s(Blocks.cobblestone), s(Blocks.cobblestone), s(Blocks.cobblestone), s(Blocks.cobblestone), s(CCItems.blueprint), s(Blocks.cobblestone), s(Blocks.cobblestone), s(Blocks.redstone_block), s(Blocks.cobblestone)}, s(CCBlocks.combiner), lineXPos - 9, lineYPos);
            ...
            rightPage();
            drawAlignedString(f("item.template.name"), Alignment.CENTER);
            drawCraftingRecipe(new ItemStack[]{s(Items.paper), s(CCItems.bluestone), s(CCItems.bluestone), s(Items.paper)}, s(CCItems.blueprint), lineXPos + 11, lineYPos);
            drawEmptyString();
            drawAlignedString(f("tile.soulExtractor.name"), Alignment.CENTER);
            ...
            drawAlignedString(f("item.spawnEggTemplate.name"), Alignment.CENTER);
            drawCraftingRecipe(new ItemStack[]{s(Items.egg), null, null, s(CCItems.blueprint)}, s(CCItems.spawn_egg_blueprint), lineXPos + 13, lineYPos);
            drawEmptyString();
            drawAlignedString(f("tile.spawnEggCombiner.name"), Alignment.CENTER);
            drawCraftingRecipe(new ItemStack[]{s(Blocks.cobblestone), s(Blocks.cobblestone), s(Blocks.cobblestone), s(Blocks.cobblestone), s(CCItems.blueprint), s(Blocks.cobblestone), s(Blocks.cobblestone), s(Blocks.redstone_block), s(Blocks.cobblestone)}, s(CCBlocks.combiner), lineXPos - 6, lineYPos);
            rightPage();
            drawAlignedString(f("item.transmutator.name"), Alignment.CENTER);
            ...
            drawExtractingRecipe(s(CCItems.bat_wing), s(CCItems.soul, 1, 13), lineXPos, lineYPos);
            drawAlignedString(f("craftableCreatures.guide.page9.mainText.4"), Alignment.CENTER);
            drawCombiningRecipe(s(CCItems.soul, 1, 1), s(CCItems.spawn_egg_blueprint), s(Items.spawn_egg, 1, EntityList.getEntityID(new EntityCreeper(null))), lineXPos + 10, lineYPos);
            rightPage();
        ...
        for (DrawingStack stack : stacks) {
            if (stack.shouldDraw(currentPage)) {
                ...
                if (stack.holdingStack != null) {
                    ...
                    itemRender.renderItemAndEffectIntoGUI(stack.holdingStack,
                            stack.x, stack.y);
                    itemRender.renderItemOverlayIntoGUI(fontRendererObj, stack.holdingStack,
                            stack.x, stack.y, null);
...
//    }
    ...
    private void drawExtractingRecipe(ItemStack input, ItemStack output, int x, int y) {
        ...
        drawItemStack(s(CCItems.soul, 1, 0), x + 43, y + 6, false);
        drawItemStack(input, x + 6, y + 6, false);
